...
public class aes{
    ...
    private static final int in_sbox[] = {
    ...
    private static int [] KeyExpansion(int key[]){
        ...
        while(bytes < 240){
            i = 0;
            while(i < 4){
                temp[i] = key[i + bytes - 4];
                i++;
            }

            //starting new round key generation
            if(bytes % 32 == 0){
                //rotate temporary array
                int temp_byte = temp[0];
                int count = 0;
                while(count < 3){
                ...
                //s-box swap
                while(count < 4){
                    temp[count] = s_box[temp[count]];
                    count++;
                }
                //XOR with rcon value
                temp[0] = temp[0] ^ rcon[roundCount];
                roundCount++;
            }

            //extra s-box swap step for 256 bit encryption
            if(bytes % 32 == 16){
                int z = 0;
                while(z < 4){
                    temp[z] = s_box[temp[z]];
            ...
            //generate next 4 bytes of key schedule
            int z = 0;
            while(z < 4){
                key[bytes] = key[bytes - 32] ^ temp[z];
                z++;
                bytes++;
            }
        }
    ...
    private static void Inv_SubBytes(int[] state) {
        for (int i = 0; i < 16; i++) {
    ...
    private static void Inv_ShiftRows(int[] state) {
        int[] temp = new int[16];
    ...
    private static void Inv_MixColumns(int[] state) {
    ...
    private static void validate_block(String block){
        if(block.length() != 32 || !(block.matches("^[0-9a-fA-F]*$"))){
            System.out.println("ERROR: Not valid string: " + block);
            System.exit(-1);
        }
    }

    private static int [] AES_Decrypt(int state[], int byteKey[], Cipher cipher) throws NoSuchPaddingException, NoSuchAlgorithmException {
        Cipher ECB = Cipher.getInstance("AES/ECB/PKCS5Padding");
        int numberOfRounds = 14;
        int key [] = KeyExpansion(byteKey);
        int from = key.length - 16;
        int to = key.length;
        int round_key [] = Arrays.copyOfRange(key,from,to);
        from -= 16;
        to -= 16;

        AddRoundKey(state, round_key);//whitening step
        for (int i = numberOfRounds; i > 1; i--) {
            Inv_ShiftRows(state);
            Inv_SubBytes(state);
            int rounds [] = Arrays.copyOfRange(key,from,to);
        ...
        Inv_ShiftRows(state);
        Inv_SubBytes(state);
        round_key = Arrays.copyOfRange(key,from,to);
        AddRoundKey(state, round_key);
        return state;

    }

    private static int [] AES_Encrypt(int state[], int byteKey[], Cipher cipher) throws NoSuchPaddingException, NoSuchAlgorithmException {
        Cipher ECB = Cipher.getInstance("AES/ECB/PKCS5Padding");
        int numberOfRounds = 14;
        int key [] = KeyExpansion(byteKey);
        int from = 0;
    ...
    public static void main(String[] args) throws NoSuchPaddingException, NoSuchAlgorithmException {
        ...
        //key shedule in bytes
        int byteKey [] = new int [240];
        int count = 0;
        int index = 0;

        //insert first 32 bytes of key schedule, rest of schedule generated during key expansion
        while(count < binary.length()){
            byteKey[index] = Integer.parseInt(binary.substring(count,count + 8),2);
            index++;
            count += 8;
        }
        ...
        //begin encryption/decryption: 16 byte blocks (128 bits)
        while(scan_InputFile.hasNextLine()){
            ...
            //create array containing all 16 bytes of the block
            int byteBlock [] = new int [16];
            index = 0;
            count = 0;
            while(count < temp.length()){
                byteBlock[index] = Integer.parseInt(temp.substring(count,count + 8),2);
                index++;
                count += 8;
            }

            int [] outputBlock;
            outputBlock = option.equalsIgnoreCase(encrypt)?AES_Encrypt(byteBlock,byteKey, ECB):AES_Decrypt(byteBlock,byteKey, ECB);

            //convert output bytes to hex
            String outputLine = "";
            for (int anOutputBlock : outputBlock) {
                String hexChar = Integer.toHexString(anOutputBlock);
                if (hexChar.length() == 1) {
                    hexChar = "0" + hexChar;
                }
                outputLine = outputLine + hexChar;
            }
            //store lines
            list.add(outputLine);
        }
        ...
        if(option.equalsIgnoreCase(encrypt)){
            outputPath = Paths.get(inputFile + ".enc");
        }else{
            outputPath = Paths.get(inputFile + ".dec");
        }

        try{
            Files.write(outputPath,list,Charset.forName("UTF-8"));
